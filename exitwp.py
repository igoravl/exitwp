    ...
    def parse_items():
        ...
        for i in xml_items:
            ...
            if (postType == 'attachment'):
                blogAttachments[gi('wp:post_id')] = gi('guid')
                continue
            ...
            if body is not None:
                ...
                try:
                    for meta in i.findall('./' + ns['wp'] + 'postmeta'):
                        key = meta.find('./' + ns['wp'] + 'meta_key').text
                        if key == '_thumbnail_id': 
                            thumbId = meta.find('./' + ns['wp'] + 'meta_value').text
...
def write_jekyll(data, target_format):
    ...
    def get_item_path(item, dir=''):
        ...
        if item['type'] == 'post':
            ...
            filename_parts.append(uid[0:4])
            filename_parts.append('/')
            filename_parts.append(uid[5:7])
            filename_parts.append('/')
            filename_parts.append(uid[11:])
            filename_parts.append('/')

            if (not os.path.exists(''.join(filename_parts))):
                os.makedirs(''.join(filename_parts))

            filename_parts.append(date + '-Post')
    ...
    def get_attachment_path(src, dir, dir_prefix='_posts'):
        try:
        ...
        target_dir = os.path.normpath(blog_dir + '/' + dir_prefix + '/' + dir[0:4] + '/' + dir[5:7] + '/' + dir[11:])
    ...
    for i in data['items']:
        ...
        if download_images:
            for img in i['img_srcs']:
                ...
                img_path = get_attachment_path(img, i['uid'])
                url = urljoin(data['header']['link'], img.encode('utf-8'))
                print '  * Image: ' + url + '... ',
                try:
